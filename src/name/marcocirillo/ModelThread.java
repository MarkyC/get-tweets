...
public class ModelThread extends Thread /* SwingWorker<Float, Integer> */{
	 ...
	 * 
	 * @param model
	 *          - the Model to run in a separate thread
	 */
	...
	public void run() {
		...
		for (int i = 1; i < ((float) (model.getMaxStatuses() / model.getStatusPerPage())); i++) {
			...
			model.writeStatusesToFile(statuses, (i == 1) ? true : false);
		...
		JOptionPane.showMessageDialog(model.getView(), "Tweets exported: " + counter);
	}

	/*
	 * @Override protected Float doInBackground() throws Exception { for (int i =
	 * 1; i < ((float) (model.getMaxStatuses()/model.getStatusPerPage())); i++) {
	 * // Get the next 100 statuses from the specified user List<Status> statuses
	 * = null;
	 * 
	 * statuses = model.getStatusPageList(i);
	 * 
	 * if (statuses == null) {
	 * DebugCrash.printDebugInfo("Failed to get status list", new
	 * NullPointerException()); }
	 * 
	 * model.writeStatusesToFile(statuses);
	 * 
	 * //this.updateProgress((float) model.getStatusPerPage() /
	 * model.getMaxStatuses()); this.oldProgress = (float)
	 * model.getStatusPerPage() / model.getMaxStatuses(); }
	 * 
	 * 
	 * model.finishAndShow(); return this.oldProgress; }
	 * 
	 * private void updateProgress(float f) { // TODO Auto-generated method stub
	 * 
	 * }
	 */
